"""Downloads the FineWeb-Edu (sample-10BT) dataset, tokenizes it with the GPT-2 BPE encoding,
and writes the tokens to disk as fixed-size uint16 shards."""
    ...
    with mp.Pool(num_workers) as pool:
        for document_tokens in pool.imap(tokenize_document, dataset, chunksize=16):
            ...
            if tokens_in_shard + len(document_tokens) < SHARD_SIZE:
                shard_buffer[tokens_in_shard:tokens_in_shard + len(document_tokens)] = document_tokens
...
def tokenize_document(document):
    ...
    tokens = [END_OF_TEXT_TOKEN]  # delimits documents from one another in the token stream
